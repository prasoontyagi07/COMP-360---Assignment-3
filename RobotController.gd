...
@export var boundary_x: float = 9.0  # Max distance from center on X-axis
@export var boundary_z: float = 9.0  # Max distance from center on Z-axis
...
func _physics_process(_delta: float) -> void:
	...
	if get_tree().paused: # Added check in case game is paused
		return
	...
	# --- NEW: ENFORCE BOUNDARIES ---
	var pos = global_position
	
	pos.x = clamp(pos.x, -boundary_x, boundary_x)
	pos.z = clamp(pos.z, -boundary_z, boundary_z)
...
func _get_move_input_dir() -> Vector3:
	...
	if Input.is_action_pressed("left"):
		dir -= right # <-- FIXED: Should subtract the 'right' vector
